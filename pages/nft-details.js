    ...
    <div className="flexBetweenStart my-5">
      <div className="flex-1 flexStartCenter">
        ...
        <div className="flexCenterStart flex-col ml-5">
          <p className="font-poppins dark:text-white text-nft-black-1 font-semibold text text-sm minlg:text-xl">{shortenAddress(nft.seller)}</p>
          <p className="font-poppins dark:text-white text-nft-black-1 font-semibold text text-sm minlg:text-xl">{nft.name}</p>
        </div>
      </div>
      <div>
        <p className="font-poppins dark:text-white text-nft-black-1 font-normal text text-sm minlg:text-xl">{nft.price} <span className="font-semibold">{nftCurrency}</span></p>
      </div>
    </div>
    <div className="flexBetween mt-10">
      <p className="font-poppins dark:text-white text-nft-black-1 font-normal text text-base minlg:text-xl">Total</p>
      <p className="font-poppins dark:text-white text-nft-black-1 font-normal text text-sm minlg:text-xl">{nft.price} <span className="font-semibold">{nftCurrency}</span></p>
    </div>
  </div>
);
          ...
          <Modal
            ...
            body={(
              <div className="flexCenter flex-col text-center" onClick={() => setSuccessModal(false)}>
                ...
                <p className="font-poppins dark:text-white text-nft-black-1 font-normal text text-sm minlg:text-xl mt-10">You successfully purchased
                  <span className="font-semibold text-success-color"> {nft.name}
                    <span className="text-white"> from</span>
                    <span className="font-semibold text-success-color"> {shortenAddress(nft.seller)}</span>
                  </span>
                </p>
              </div>
            )}
